...
{
    int timeout = 10000; // maximum ms to wait between characters received before aborting as timed out
    QByteArray rawData;
    int errorCount = 0;
    QTime elapsed;
    char byte;
...
Comm::ErrorCode Comm::SendPacket(const QByteArray& sendPacket)
{
    ...
    // send the STX
    qint64 written = serial->write(sendPacket.left(1));
    ...
    written = serial->write(sendPacket.mid(1));
    serial->waitForBytesWritten(1);
    if(written != (sendPacket.size() - 1))
    {
        return CouldNotTransmit;
    }
...
Comm::ErrorCode Comm::SendGetPacket(const QByteArray& sendPacket, QByteArray& receivePacket, int retryLimit, int timeOut)
{
    ...
    while(retryLimit-- >= 0)
    {
        qint64 written = serial->write(sendPacket.mid(1));
        serial->waitForBytesWritten(1);
        if (written != (sendPacket.size() - 1))
        {
...
Comm::BootInfo Comm::ReadBootloaderInfo(int timeout)
{
    ...
    for(;;)
    {
        ...
        serial->waitForReadyRead(1);// VL ???
        response = serial->peek(1);
        if(response == sync)
        {
            qWarning("Received STX, sending packet. %fs", (double)elapsed.elapsed() / 1000);
            qint64 written = serial->write(sendPacket);
            serial->waitForBytesWritten(1);
            if(written != sendPacket.size())
            {
